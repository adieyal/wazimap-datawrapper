"""
Originally based on https://github.com/chekos/datawrapper but there have been substantial changes. Adding features as I go.
"""
...
class Datawrapper:
    ...
    def set_data(self, chart_id, data):
        """
    ...
    def set_minimap(self, chart_id, geometry):
        _header = self._auth_header
        _header["content-type"] = "application/json"

        topo = topojson.Topology(shape(geometry))
        js = json.loads(topo.to_json())
        js["objects"]["geom"] = js["objects"]["data"]
        del js["objects"]["data"]
        
        response = r.put(
            url=f"{self._CHARTS_URL}/{chart_id}/assets/{chart_id}.minimap.json",
            headers=_header,
            data=json.dumps(js)
        )

        print(f"Minimap response: {response.status_code}")


    def get_minimap(self, chart_id):
        _header = self._auth_header
        _header["content-type"] = "application/json"
